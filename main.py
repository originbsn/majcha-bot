...
- โปรสงกรานต์: ลด 10% วันที่ 13-15 เมษายน แต่วันจันทร์ ที่13 ร้านหยุดเปิด 14 นะคะ
- ร้านเปิดปกติตลอดช่วงสงกรานต์ วันจันทร์ที่ 13 ร้านหยุด
...
- ถ้าถามเปิดสงกรานต์มั้ย/วันหยุดพิเศษช่วงสงกรานต์: บอกว่าเปิดปกติค่ะ 11.00-22.00 น. และแจ้งโปรสงกรานต์ด้วยว่า "มีโปรลด 10% วันที่ 14-15 เม.ย.นะคะวันที่ 13 ร้านหยุดค่ะ 🎉"
- ถ้าถามโปร/ส่วนลด: บอกทั้งสองโปรพร้อมกันเลย "มีโปรวันเกิดลด 10% และโปรสงกรานต์ 13-15 เม.ย. ลด 10% ฉลองสงกานต์คะ วันที่ 13 ร้านหยุดค่ะ"
...
- โปรวันเกิด: ลด 10% ต้องแจ้งล่วงหน้าและแสดงหลักฐานวันเกิด
- โปรสงกรานต์: ลด 10% วันที่ 13-15 เมษายน วันที่ 13 ร้านหยุดรับจอง 14-15 เม.ษ
...
# Session storage พร้อม TTL 24 ชม. — ป้องกัน memory leak
...
BOOKING_KEYWORDS = [
    ...
    # เพิ่มใหม่ — warm lead signals
    'ราคา','เท่าไหร่','เท่าไร','กี่บาท',
    'เมนูมีอะไร','มีอะไรบ้าง',
    'อยู่ไหน','อยู่แถวไหน','ที่อยู่','ทางไหน','เดินทาง',
    'เปิดกี่โมง','ปิดกี่โมง','เปิดมั้ย','เปิดไหม','เปิดวันไหน',
    'มีที่จอดรถมั้ย','จอดรถ',
]
...
def get_session(user_id):
    """ดึง session และ clean up sessions เก่าที่หมดอายุ"""
    now = time.time()

    # Clean up expired sessions ทุกครั้งที่มี request
...
def build_telegram_summary(history, is_deposit):
    ...
    for i, m in enumerate(history):
        ...
        # ยอดรวม — จับหลาย pattern
        tot = re.search(r'(?:รวม|ยอด|ทั้งหมด|คิดเป็น)\D{0,8}([1-9][\d,]{2,})\s*บาท', t)
        if not tot:
            tot = re.search(r'([1-9][\d,]{3,})\s*บาท', t)  # ตัวเลข 4+ หลัก
        ...
        # ชื่อ — ดูจาก user ที่ตอบหลัง bot ถามชื่อ/เบอร์
